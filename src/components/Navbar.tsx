  ...
  return (
    <header className="sticky top-0 z-40 backdrop-blur-md bg-[#fcfaf7]/90 dark:bg-[#0b0f19]/90 border-b border-emerald-900/10 dark:border-emerald-500/15 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between min-h-16 sm:h-20 gap-2 sm:gap-4">
          ...
          <button type="button"
            ...
            className="flex items-center gap-2 sm:gap-3 cursor-pointer select-none group min-w-0 text-left"
            aria-label="Menyang Beranda"
          >
            <div className="w-9 h-9 sm:w-12 sm:h-12 shrink-0 rounded-2xl bg-gradient-to-tr from-emerald-600 to-teal-400 flex items-center justify-center text-white shadow-lg shadow-emerald-500/25 group-hover:scale-105 transition-transform">
          ...
          <nav className="hidden xl:flex items-center gap-1.5 bg-slate-200/60 dark:bg-slate-800/60 p-1.5 rounded-2xl border border-slate-300/40 dark:border-slate-700/40">
            {navItems.map((item) => {
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1 sm:py-1.5 rounded-xl sm:rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-900 dark:text-amber-200 min-w-0">
              ...
              <div className="hidden min-[390px]:block h-3.5 w-px bg-amber-400/40" />
              {isEditingName ? (
                ...
                <button
                  ...
                  className="hidden min-[390px]:flex items-center gap-1 text-[11px] sm:text-xs font-semibold text-slate-700 dark:text-slate-200 hover:underline cursor-pointer"
      ...
      <nav
        ...
        className="xl:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 dark:bg-slate-900/95 backdrop-blur-lg border-t border-slate-200/90 dark:border-slate-800 px-2 py-1.5 flex items-center justify-around shadow-[0_-4px_16px_rgba(0,0,0,0.08)]"
      >
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive = activeTab === item.id;
